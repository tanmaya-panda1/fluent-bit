...
int azure_blob_store_file_delete_ext_latest(struct flb_azure_blob *ctx, struct azure_blob_file *azure_blob_file)
{
    int ret;
    struct flb_fstore_file *fsf;
    int fd;
    ...
    if (fsf != NULL) {
        ...
        if (access(azure_blob_file->file_path, F_OK) == -1) {
            flb_plg_warn(ctx->ins, "File does not exist");
            //azure_blob_file_cleanup(azure_blob_file);
            //flb_free(azure_blob_file);
            return 0;
        }

        // Open the file for locking
        fd = open(azure_blob_file->file_path, O_RDWR);
        if (fd == -1) {
        ...
        if (flock(fd, LOCK_EX | LOCK_NB) == -1) {
            if (errno == EWOULDBLOCK) {
                flb_plg_warn(ctx->ins, "File '%s' is locked by another process, skipping deletion", azure_blob_file->file_path);
                close(fd);
                return 0;
            } else {
                flb_plg_error(ctx->ins, "Failed to lock file '%s': %s", azure_blob_file->file_path, strerror(errno));
                close(fd);
                return -1;
            }
        }
        ...
        if (ret != 0) {
            flb_plg_error(ctx->ins, "File ingested but Failed to delete file %s with size %zu", azure_blob_file->file_path, azure_blob_file->size);
            ret = -1;
        } else {
            flb_plg_debug(ctx->ins, "File ingested and Deleted file '%s' and with size %zu", azure_blob_file->file_path, azure_blob_file->size);
            ret = 0;
        }

        flb_plg_debug(ctx->ins, "Freeing memory for azure_blob_file at address: %p", (void *)azure_blob_file);
        azure_blob_file_cleanup(azure_blob_file);
        flb_free(azure_blob_file);
        azure_blob_file = NULL; // Set pointer to NULL after freeing
...
int azure_blob_store_file_delete_ext(struct flb_azure_blob *ctx, struct azure_blob_file *azure_blob_file)
{
    int ret;
    struct flb_fstore_file *fsf;
    ...
    if (fsf != NULL) {
        ...
        if (ret != 0) {
            flb_plg_error(ctx->ins, "Failed to delete file '%s': %s", azure_blob_file->fsf->name, strerror(errno));
            ret = -1;
        }else{
            flb_plg_debug(ctx->ins, "Deleted file '%s'", azure_blob_file->fsf->name);
            ret = 0;
        }
        flb_plg_debug(ctx->ins, "Freeing memory for azure_blob_file at address: %p", (void *)azure_blob_file);
        flb_free(azure_blob_file);
