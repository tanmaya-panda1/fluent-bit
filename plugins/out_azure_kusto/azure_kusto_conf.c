                                                              ...
                                                              const char *url)
{
    ...
    if (tmp) {
        ...
        if (kv) {
            ...
            if (ret != -1) {
                ret = flb_hash_table_add(kv, AZURE_KUSTO_RESOURCE_UPSTREAM_SAS, 3, tmp + 1, sas_length);
    ...
    if (sds_host) {
        flb_sds_destroy(sds_host);
    }
    if (sds_port) {
        flb_sds_destroy(sds_port);
    }
    if (kv) {
...
static struct flb_upstream_node *flb_upstream_node_create_url_ext(struct flb_azure_kusto *ctx,
                                                              struct flb_config *config,
                                                              const char *url)
{
    int ret;
